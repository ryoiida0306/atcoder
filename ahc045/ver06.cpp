...
vector<P> response(vector<int>& c, vector<P>& X){
    vvi correct_dist((int)c.size(), vi((int)c.size(), INF));
    rep(i,(int)c.size()) correct_dist[i][i] = 0;

    rep(i,(int)c.size())rep(j,(int)c.size()){
        if(i == j) continue;
        correct_dist[i][j] = (int)sqrt((X[c[i]].first - X[c[j]].first)*(X[c[i]].first - X[c[j]].first) + (X[c[i]].second - X[c[j]].second)*(X[c[i]].second - X[c[j]].second));
    }

    dsu G((int)c.size());
    vector<P> res;
    priority_queue<pair<int, P>, vector<pair<int, P>>, greater<pair<int, P>>> pq;
    rep(i,(int)c.size())for(int j = i+1; j < (int)c.size(); j++){
        pq.push({correct_dist[i][j], {i, j}});
    }
...
int main(){
    ...
    int query_idx = 0;
    rep(i, m){
        // if(g[i].first != (int)c[g[i].second].size()){
        //     cout << g[i].first << " " << c[i].size() << endl;
        //     cout << "error" << endl;
        //     return 0;
        // }
        if(g[i].first > l)continue;
        if(g[i].first < 3)break;
        if(query_idx >= q)break;

        cout << "? " << g[i].first << " ";
        for(auto cc : c[g[i].second]){
            cout << cc << " ";
        }
        cout << endl;

        if(debug){
            ab[g[i].second] = response(c[g[i].second], X);
            for(auto aa : ab[g[i].second]){
