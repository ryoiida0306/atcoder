...
int main(){
    ...
    while((int)pq.size()){
        ...
        rep(i,n){
            ...
            to_uv.push_back({min(to_u[i], to_v[i]), i, (to_u[i] < to_v[i] ? u : v)});
        }
        ...
        rep(i,g[g_idx].first-2){
            to_uv_sub.push_back(to_uv[i]);
        }

        shuffle(to_uv_sub.begin(), to_uv_sub.end(),ggg);

        int baffa_size = (g[g_idx].first-2)/5;
        vi baffa(baffa_size);
        // int baffa_idx = 0;

        rep(i,baffa_size){
            baffa[i] = get<1>(to_uv_sub[i]);
        }
        ...
        rep(i,g[g_idx].first-2){
            ...
            if(query_count < q && (int)ss.size() > 1){
                query_count++;
                // cout << "? 3 " << baffa[0] << " " << baffa[1] << " " << baffa[2] << endl;
                cout << "? " << (int)ss.size() << " ";
                for(auto sss : ss){
                    cout << sss << " ";
                }
